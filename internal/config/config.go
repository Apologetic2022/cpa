...
type Config struct {
	...
	// CursorKey defines a list of Cursor API key configurations as specified in the YAML configuration file.
	// Each key is exchanged for short-lived Agent Connect JWTs at runtime.
...
// CursorKey represents the configuration for a Cursor user API key (crsr_...).
// The key is exchanged for short-lived Agent Connect access tokens at runtime
// via /auth/exchange_user_api_key, so no OAuth login is required.
type CursorKey struct {
	// APIKey is the Cursor user API key used for the token exchange.
	APIKey string `yaml:"api-key" json:"api-key"`
	...
	// Prefix optionally namespaces models for this credential (e.g., "teamA/gpt-5.4").
	Prefix string `yaml:"prefix,omitempty" json:"prefix,omitempty"`

	// BaseURL overrides the default Cursor API endpoint (https://api2.cursor.sh) when set.
