...
// The credential refresh path is deleted from this fork (relay docs 6.6.1/F3):
// ProviderExecutor no longer carries Refresh — OAuth token lifecycles are owned by
// the accounts' real clients, never by the gateway.
...
type Manager struct {
	...
	// The credential auto-refresh path (loop, scheduling, per-auth refresh locks)
	// is deleted from this fork (relay docs 6.6.1/F3): the account's real CC agent
...
// The auto-refresh loop itself is deleted from this fork (relay docs 6.6.1/F3):
// the account's real CC agent owns the OAuth token lifecycle; the gateway never
// rewrites credentials and never initiates a refresh.
